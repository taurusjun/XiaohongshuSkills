...
【标题要求】
重新创作一个中文标题，优先级如下：
① 先写人物冲突：用文中出现的真实人名或团体名，点出"谁 vs 谁"或"同一人的两面"，制造张力。
② 再带出事件背景：用冒号或破折号连接，补充一句话说明这场冲突发生在什么情境里。
格式参考："人物A×人物B的冲突 — 事件背景简述"，不超过25字。
人名/团体名统一用简体中文（如「桥本环奈」不写「橋本環奈」）。
禁止只写事件不写人物，禁止使用"深度""探讨""揭秘"等套话。
参考原标题翻译：{title_zh}
