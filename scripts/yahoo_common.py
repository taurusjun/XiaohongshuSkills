...
- 标题要有故事感，不要平铺直叙。给读者一条时间线或一个变化：
  ...
  好：「梅泽美波毕业首次出演朗读剧」← 毕业→转型→新舞台，有叙事线
- 20字上限，超了砍修饰词
- 标题统一用简体中文，日式汉字转简体（実→实、愛→爱、島→岛、澤→泽、櫻→樱、結→结、黒→黑、會→会、園→园、戦→战、円→圆、関→关、気→气、楽→乐、読→读、語→语、長→长、門→门、間→间、時→时、開→开、個→个、寫→写）
...
- 以下词/句式一个都不能出现：破防/慌了/沉默了/画风突变/评论区炸了/粉丝沸腾了/全网震惊/太值了/绝了/炸裂/天花板/必看/不看你亏/建议收藏/干货/下一秒/那一刻/宝子们/姐妹们/太美了吧/震惊/太/超/极
- 标题不要重复原新闻标题的句式，用自己的话重新表达
- 日文直译必须转为中文自然表达。例如「全部やり切った」译成"问心无愧""没有遗憾""拼尽全力了"，不能译成"全部做完了"。任何日文原句直译成中文后若不通顺，必须意译，不要保留日语句式
- 标题统一用简体中文，日式汉字必须转简体。必查清单：実→实、愛→爱、島→岛、澤→泽、櫻→樱、結→结、黒→黑、會→会、園→园、戦→战、円→圆、関→关、気→气、楽→乐、読→读、語→语、長→长、門→门、間→间、時→时、開→开、個→个、寫→写
- 不要用「！」，用句号或问号

质感要求：
- 必须包含剧情感、冲突感或情绪表达三者之一。平铺直叙不通过
- 把原文中最反常、最扎心、最有画面感的那个点提取出来作为标题核心
- 好标题像电影预告片，勾人点进去；差标题像新闻摘要，看标题就够了


【质感要求】
- 读完正文全文，找到最反常/最扎心/最有画面感的一个具体细节塞进标题。只概括=失败
- 判据：标题里有没有原文的某个具体数字、原话、或场景描写？没有→重写
- 必须包含剧情感、冲突感或情绪表达。平铺直叙不通过
- 好标题像电影预告片，勾人点进去；差标题像新闻摘要，看标题就够了

严格只输出一行JSON。不要任何思考过程。你的整个回复必须是且仅是一个JSON对象。
格式：{{"title":"生成的中文标题"}}"""
...
def generate_title_only(title_ja: str, content_ja: str) -> str:
    ...
    result = call_litellm(prompt, system_prompt="只输出JSON。", max_tokens=1000, temperature=1.0, thinking_disabled=True)
    if not result:
        return ""
    # 解析 JSON
    try:
