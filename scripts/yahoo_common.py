...
【标题要求】
重新创作一个中文标题，不超过25字，全部简体中文，人名不加【】括号。

根据内容类型选择结构——严禁默认套用"A×B——描述"模板：
- 访谈/对话类（问答为主）：用人物聚焦型，如「山口阳世谈毕业：枕着后辈的膝盖睡着的人」
- 对比/冲突类（两人命运明显对立）：可用 A×B——差异，但破折号后不超过8字
- 单一事件类：直接描述事件，如「伯乐一句话，救了沉底十年的演员」
- 争议/反转类：用悬念句式，如「骂完东大生，他被自己的旧账淹没了」

禁止出现：×和——同时出现、"深度""探讨""揭秘""光与影""温柔与坚韧"及类似抽象对仗套话。
参考原标题翻译：{title_zh}

【导语要求】
一句话金句，从制度/规则的角度切入：聚焦"允许拍摄之后发生了什么、谁来负责"这个层面，而不是重复人物命运的对比（标题已说了）。
要求：不超过30字，不出现标题已有的词语，揭示规则或制度上的空白/失控/悖论。
示例风格：「开放拍摄的规则，没有人为快门之后的事负责。」
