...
"""
    ...
    from config.prompts import TITLE_RULES_NEWS, TITLE_QUALITY
...
再根据文章中的具体人名/团体名补充精准标签）"""
    ...
    raw_seo = last_section("标题") or last_section("SEO标题")  # 兼容旧字段名
    if raw_seo:
...
def generate_story_article(title_ja: str, title_zh: str, body_ja: str,
    ...
    angle_note = f"\n【内容角度】今日重点关注：{angle}\n" if angle else ""
    from config.prompts import TITLE_RULES_STORY, TITLE_QUALITY
...
def _build_title_prompt(title_ja: str, content_ja: str,
    ...
    # 正文上下文：只用日文原文（导语+结语已是中文精华，不需要再加中文正文摘要）
    content_block = f"日文正文（前500字）：{content_ja[:500]}" if content_ja else ""
