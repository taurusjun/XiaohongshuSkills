...
# 主题引擎目录 = scripts/，config 由调用方提供
#  config.json 加载跳过，使用默认值

# 引擎配置（不依赖外部 config.json，由调用方通过参数覆盖）
OUTPUT_DIR = Path("/tmp/wechat-format")
VAULT_ROOT = Path.home() / "Documents"
...
# ── XiaohongshuSkills 桥接函数 ──────────────────────────────────────────

_IMG_PATTERN = re.compile(r'【(?:图片|推文)(\d+)[：:]([^】]*)】')


def xhs_img_to_markdown(content: str, img_url_map: dict = None) -> str:
    """将 XiaohongshuSkills 的【图片N：/path】标记转为标准 Markdown 图片。

    Args:
        content: 含【图片N：/path】标记的正文
        img_url_map: {local_path: wx_url} 映射。如果提供，图片引用 CDN URL；
    ...
    def _replace(m):
        ...
        if len(paths) > 1:
            parts = []
            for p in paths:
                url = img_url_map.get(p, p) if img_url_map else p
                parts.append("![图%s](%s)" % (idx, url))
            return "\n".join(parts)

        path = paths[0]
        url = img_url_map.get(path, path) if img_url_map else path
        return "![图%s](%s)" % (idx, url)
