...
#### Scenario: 完整周报生成（数值部分模板渲染，摘要部分 LLM 生成）
- **WHEN** `reflection_runner.py` 运行且有 ≥ 1 篇本周发布文章含实发数据
- **THEN** 周报分两部分生成：
  - **模板渲染部分（不调用 LLM）**：本周发布篇数、总浏览、总收藏、各话题对比表、体裁分布表、环比数字——直接从 DB 聚合后填充 Markdown 模板，避免数值幻觉
  - **LLM 生成部分（temperature=0.5，max_tokens=500）**：「本周亮点摘要」（50-100字）和「下周建议」（50-100字）两段自然语言，输入是模板渲染后的数值摘要，LLM 只做文字润色和归纳，不自行生成数字
