...
**正文输入采用动态 token budget 截断，而非固定字符数：**

评分 prompt 总 token 预算约 3000：
- 维度定义（20个含 example_0_5）≈ 2000 tokens
- 剩余 budget ≈ 1000 tokens → 约 **800 个日文字符**

正文输入：`content_ja[:800]`（日文字符，约等于 1000 tokens）

对分页长文（`is_long_form=True`）同样截断 800 字——长文已被标记，LLM 知道这是长文内容，评分时不需要看全文才能判断各维度。`原创度`、`啰嗦重复`、`有用信息` 等需要全文视角的维度，800 字已能覆盖 2-3 个段落，显著优于原来的 200 字。

token budget 总计：2000（定义）+ 1000（内容）+ 200（标题/评语）≈ 3200 tokens input。按每天 10 篇 × 1.5 次平均评分 = 48,000 tokens/天，可接受。

#### Scenario: 动态 budget 截断覆盖更多内容
- **WHEN** 文章正文前 200 字是引言，精华在中后段
- **THEN** `content_ja[:800]` 覆盖更多段落，`原创度`、`有用信息`、`啰嗦重复` 等需要全文视角的维度判断更准确

#### Scenario: 长文评分不需要全文
- **WHEN** 文章 `is_long_form=True`，完整正文 2000+ 字
- **THEN** 仍截断到 800 字传给评分 LLM；`story` 体裁适用性已通过 `is_long_form` 标记确定，不依赖全文评分
