...
- [x] 2.1 实现 `_is_recent(text: str, hours: int = 48) -> bool`，处理所有 XHS 时间字符串格式
- [x] 2.2 编写 `tests/test_gap_trend_detection.py`：TC-P1, TC-P2

## 3. upsert_topic_performance trend_only 路径（scripts/sqlite_db.py）

- [x] 3.1 新增 `trend_only: bool = False` 参数；当 True 时跳过 rolling average 更新，只写 baseline 和 trend_signal
- [x] 3.2 编写测试：TC-P3（trend scanner 调用后 avg_saves 不变）

## 4. scan_topic_trends 双路扫描重构（scripts/xhs_trend_scanner.py）

- [x] 4.1 Pass 1：综合排序（sort="general"），计算 baseline_saves（截尾均值）、image_ratio、avg_title_len
- [x] 4.2 Pass 2：最新+一天内（sort="newest", time_filter="1day"，方案B），is_fresh = feeds>=5
- [x] 4.3 废弃 sort="最多收藏"，改为明确的 sort="general"
- [x] 4.4 调用 upsert_topic_performance 时传 saves=0, comments=0, trend_only=True
- [x] 4.5 编写测试：TC-P4（is_fresh 阈值验证）

## 5. agent_planner.py 规划层使用 is_fresh

- [x] 5.1 修改 recommend_post_times：is_fresh=True 话题优先分配最早时段
- [x] 5.2 修改 explore 候选排序：is_fresh=True 优先
- [x] 5.3 编写测试：TC-PL1, TC-PL2
